...
const blogTemplates: Record<string, Array<{ title: string; content: string; excerpt: string }>> = {
  "Cryptography": [
    {
...
const defaultTemplate = (conceptName: string, index: number): { title: string; content: string; excerpt: string } => {
  const titles = [
...
async function main() {
      ...
      const blog = await prisma.blogPost.create({
        data: {
          ...
          conceptIds: Array.from(
            new Set([concept.id, relatedConcept.id])
          ),
          relatedIds: [],
          embedding: null,
        },
  ...
  const linkConcepts = async (a: string, b: string) => {
